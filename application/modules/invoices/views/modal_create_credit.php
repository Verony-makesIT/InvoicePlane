                ...
                <select name="invoice_group_id" id="invoice_group_id" class="hidden">
                    <?php foreach ($invoice_groups as $invoice_group) { ?>
                        <option value="<?php echo $invoice_group->invoice_group_id; ?>"
                            <?php if (get_setting('diff_cni_format') == 0 && get_setting('default_invoice_group') == $invoice_group->invoice_group_id) {
                                echo 'selected="selected"';
                                $credit_invoice_group = htmlsc($invoice_group->invoice_group_name);
                             } elseif (get_setting('diff_cni_format') == 1 && get_setting('default_creditnote_group') == $invoice_group->invoice_group_id) {
                                echo 'selected="selected"';
                                $credit_invoice_group = htmlsc($invoice_group->invoice_group_name);
                            } ?>>
